...
export class HellionYTDLResolver extends HellionSingleMusic {
    public async resolve(music: string): Promise<HellionMusicResolved | null> {
        ...
        return { title: info.video_details.title || "Unresolvable title", resolvable: music, duration: info.video_details.durationInSec * 1000 };
    }
...
export class HellionYTPLResolver extends HellionBulkMusic {
    public async bulk(music: string): Promise<HellionMusicResolved[] | null> {
        ...
        for (let item of await pl.all_videos()) {
            res.push({ title: item.title || "Unresolvable title", resolvable: item.url, duration: item.durationInSec * 1000 })
        }
        return res;
    }
...
export class HellionYTSRResolver extends HellionSingleMusic {
    public async resolve(music: string): Promise<HellionMusicResolved | null> {
        ...
        if (info.length == 0)
            return null;
        return { title: info[0].title || "Unresolvable title", resolvable: info[0].url, duration: info[0].durationInSec * 1000 };
    }
