...
import { GuildMember, EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, MessageComponentInteraction, Client } from "discord.js";
...
export class HellionCommand extends commandHandler.HellionCommandListener {
    ...
    public async run(event: commandHandler.HellionCommandEvent, data: any): Promise<void> {
        ...
        let creator: any = event.user;
        creator.kbUsername = (event.member as GuildMember).nickname || event.user.username;

        let collector = event.channel?.createMessageComponentCollector({ filter: (i: MessageComponentInteraction) => i.customId == "join-knuckles" && i.user.id !== event.user.id, time: 30000, max: 1 });
        collector?.on('collect', async (i: MessageComponentInteraction) => {
            i.update({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setFooter({ text: "Hellion Warden by Nashira Deer", iconURL: event.client.user?.avatarURL() || '' })
                        .setTitle("Hellion Warden // Knucklebones")
                        .setDescription(`**${i.message.member?.nickname || i.user.username}** ACCEPTED THE CHALLANGE, STARTING THE GAME!.`)
                ],
                components: [
                    new ActionRowBuilder<ButtonBuilder>()
                        .addComponents(
            ...
            let enemy: any = i.user;
            enemy.kbUsername = event.guild?.members.cache.get(enemy.id)?.nickname || i.user.username;
        ...
        let dicemsg = await kb.channel?.send({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setTitle("Hellion Warden // Knucklebones")
                    .setDescription(`Rolling a dice for **${kb.currentPlayer.kbUsername}**...`)
            ]
        });
    ...
    await kb.channel?.send({
        embeds: [
            new EmbedBuilder()
                ...
                .setFooter({ text: "Hellion Warden by Nashira Deer", iconURL: client.user?.avatarURL() || '' })
                .setTitle("Hellion Warden // Knucklebones")
                .setDescription(`**${kb.players[0].kbUsername}:**\n${createTable(kb.table(kb.players[0].id))}\n**${kb.players[1].kbUsername}:**\n${createTable(kb?.table(kb.players[1].id))}\n\nI can't know the winner by now.`)
        ]
    });
}
        ...
        let msg = await kb.channel?.send({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setFooter({ text: "Hellion Warden by Nashira Deer", iconURL: client.user?.avatarURL() || '' })
                    .setTitle("Hellion Warden // Knucklebones")
                    .setDescription(`**${kb.players[0].kbUsername}:**\n${createTable(kb.table(kb.players[0].id))}\n**${kb.players[1].kbUsername}:**\n${createTable(kb.table(kb.players[1].id))}\n\nIt's **${kb.currentPlayer.kbUsername}** turn!\nYou rolled a ${d6}!`)
            ],
            components: [
                new ActionRowBuilder<ButtonBuilder>()
                    .addComponents(
                        ...
                        new ButtonBuilder()
                            ...
                            .setStyle(ButtonStyle.Primary)
                            .setDisabled(kb?.checkColumnFull(3)),
                    )
            ]
        });
            ...
            let msg2 = await i.channel?.send({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setFooter({ text: "Hellion Warden by Nashira Deer", iconURL: client.user?.avatarURL() || '' })
                        .setTitle("Hellion Warden // Knucklebones")
                        .setDescription(`You will select column '${columnId}', are you sure?`)
                ],
                components: [
                    new ActionRowBuilder<ButtonBuilder>()
                        .addComponents(
